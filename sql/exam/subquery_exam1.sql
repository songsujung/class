...
from emp
where deptno=(select * from dept where loc='DALLAS')
;


-- 55. KING에게 보고하는 사원의 이름과 급여를 표시하시오.
-- emp => mgr(상사) => 사원의 mgr 컬럼에 KING사원의 사원번호를 가지고 있으면 => KING 상사이다!
...
select empno, ename, sal
from emp
where sal > (select avg(sal) from emp) and deptno in (select * from emp where ename like '%M%')
;
