...
select deptno, (select dname from dept where deptno=emp.deptno) as dname
from emp
where ename = 'SCOTT'
;
...
--스칼라 부속질의(scalar subquery) : select 절에 쓸 수 있는 서브쿼리, 단일값(징의의 결과가 단일행 단일열)
select o.custid,
...
-- 인라인 뷰 : from절 뒤에 오는 서브쿼리, 가상의 테이블, 테이블처럼 사용
--일반
...
--주어진 문제가 3000 이상 받는 사원이 소속된 부서(10번, 20번)와 동일한 부서에서 근무하는 사원
...
--대한민국에 거주하는 고객에게(서브쿼리) 판매한 도서의 총 판매액을 구하시오.(메인쿼리)
...
-- exists, not exists
-- 상관관계가 반드시 필요한 서브쿼리가 조건의 결과로 사용 : select의 결과가 존재하는지 여부에 따라 결과 처리
